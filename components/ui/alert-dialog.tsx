...
>(({ className, ...props }, ref) => (
  <AlertDialogPortal>
    ...
    <AlertDialogPrimitive.Content
      ref={ref}
      className={cn(
        // The alert dialog adopts glassmorphism for a modern, frosted effect.
        // A semi‑transparent card background combined with a backdrop blur
        // draws the user’s attention and adds depth【172484293626487†L130-L167】. A light border
        // delineates the edges and dark‑mode variants ensure sufficient
        // contrast. This change modernises the alert dialog without
        // compromising readability or accessibility.
        "fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border border-border/40 bg-card/75 backdrop-blur-md p-6 shadow-lg dark:bg-card/50 dark:border-border/30 duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] sm:rounded-lg",
        className,
      )}
      {...props}
    />
